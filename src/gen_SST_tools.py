...
def addSSTPerturbationMethod2(
    ...
    for i in range(file_cnt):
        ...
        if i == 0:
            init_SST_obs = ds["SST"].to_numpy().copy()

        obs_SST = ds["SST"].to_numpy().copy()
        lnd_mask = obs_SST == 0
        ocn_mask = np.logical_not(lnd_mask)
        ...
        pred_SST = init_SST_obs + eta * clim_SST_diff
        new_SST  = (1.0 - epsilon) * obs_SST + epsilon * pred_SST
